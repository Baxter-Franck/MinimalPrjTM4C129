...
#ifdef MLKJSLMQJSM // Sample of procedure to convert a float to ASCII string
static int snprintf_fp( char destination[], size_t available_chars, int decimal_digits, float source_number )
{
        ...
        if(*pcString == '%')
        {
            ...
            switch(*pcString++)
            {
...

                {
                    ...
                    // Sample of code : https://stackoverflow.com/questions/905928/using-floats-with-sprintf-in-embedded-c
                    ...
                    // Get the value from the varargs.
                    //
                    float fValue = (float) va_arg(vaArgP, double /*float*/);

                    //ui32Pos = snprintf_fp( pcBuf, sizeof(pcBuf), ui32Count, fValue );
                    ...
                    // Handle rounding
                    for( i = ui32Count ; i > 0 ; --i )
                    {
                        zeros *= 10;
                    }

                    source_num = (uint64_t)( ( fValue * (float)zeros ) + 0.5f );

                    // Determine sliding divider max position.
                    div_amount = zeros;       // Give it a head start
                    while( ( div_amount * 10 ) <= source_num )
                    {
                        div_amount *= 10;
                    }
                    ...
                    while( div_amount > 0 )
                    {
                        whole_number = source_num / div_amount;
                        if( ui32Pos < (int)sizeof(pcBuf) )
                        {
                            pcBuf[ ui32Pos ] = '0' + (char)whole_number;
                            ++ui32Pos;
                            if( ( div_amount == zeros ) && ( zeros > 1 ) )
                            {
                                pcBuf[ ui32Pos ] = '.';
